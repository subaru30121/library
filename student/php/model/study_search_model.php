...
class studySearchModel {
	...
	// 蔵書名のバリデート
	function themeCheck($theme = null, $errorArray = null) {
	
		// 引数の確認
		if (!isset($theme) || !isset($errorArray)) {
			// 入っていなければ終了
			return false;
		}
		
		// エラーメッセージの初期化
		$errorArray['theme']['message'] = null;
		// フラグの初期化
		$errorArray['theme']['flg'] = false;
		// 入力されているか確認
		if (!empty($theme)) {
			// 入力されている場合
			// 文字数取得
			$themeNum = mb_strlen($theme, 'UTF-8');
			// 文字数確認
			if ($themeNum <= 100) {
				// 上限を超えていない場合
				$errorArray['theme']['flg'] = true;
			} else {
				// 上限を超えていた場合
				$errorArray['theme']['message'] .= "文字数が多いです\n";
			}
		} else {
			// 入力されていない場合
			$errorArray['theme']['flg'] = true;
